...
void timer(int){
	...
	lobes[ind]=xtemp+(parllelvec[0]*(rpforctemp/no_of_lobes));//rpforctemp as t
	lobes[ind+1]=ytemp+(parllelvec[1]*(rpforctemp/no_of_lobes));
	lobes[ind+2]=ztemp+(parllelvec[2]*(rpforctemp/no_of_lobes));
	lobes[ind]+=xprevind;
	...
	while(prevradveclen1!=radius){
	   ...
	   if(prevradveclen1<radius){
		...
		lobes[ind+2]=lobes[ind+2]+(radiusparlvec[2]*t);}
	      if(abs(prevradveclen2-prevradveclen1)<abs(prevradveclen1-radius)){
		t=t+(t/20);//t/20 to avoid exploding values
		cout<<" elseelse "<<t;
	   	lobes[ind]=lobes[ind]+(radiusparlvec[0]*t);
	   	lobes[ind+1]=lobes[ind+1]+(radiusparlvec[1]*t);
		lobes[ind+2]=lobes[ind+2]+(radiusparlvec[2]*t);}
